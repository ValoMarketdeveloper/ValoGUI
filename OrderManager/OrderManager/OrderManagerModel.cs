    ...
    public class OrderManagerModel
    {
        ...
        public void PrepareForShipping(OrderData ord)
        {
            try
            {
                ...
                sb.Append("p.UPC = '").Append(ord.OrdInfo.UPC).Append("' ");
                sb.Append("and OrderStatus = ").Append((int)OrderStates.Archive).Append(" ");
                var tmp = GetOrdersByState((int)OrderStates.Archive, sb.ToString()).ToList();
                tmp.ForEach(page =>
                {
                    if (page != null)
                    {
                        page.ToList().ForEach(r =>
                        {
                            orders.Add(r);
                        });
                    }
                ...
                sb.Clear();
                sb.Append("p.UPC = '").Append(ord.OrdInfo.UPC).Append("' ");
                sb.Append(" and OrderStatus = ").Append((int)OrderStates.ShipReady).Append(" ");
                tmp = GetOrdersByState((int)OrderStates.ShipReady, sb.ToString()).ToList();
                tmp.ForEach(page =>
                {
                    if (page != null)
                    {
        ...
        public void UpdateOrder(OrderData order, string field)
        {
            ...
            if (field == "SelectedStatus" && (order.OrdInfo.OrderStatus == (int)OrderStates.Archive || order.OrdInfo.OrderStatus == (int)OrderStates.ShipReady))
            {
